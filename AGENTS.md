...
├── appointments/           # Appointment / booking registry (secretary ↔ pastor)
...
- **Appointment/booking registry `src/appointments/` — secretary↔pastor pairing at `/api/v1/appointments`.** A simple CRUD registry (a record: title, scheduled date/time, secretary, pastor, location, notes, status) — not a threaded request. Permission-gated for secretary + pastor roles (`appointments:create/read/update/delete`) with `super_admin` locked to all. **Pairing scope** (from `AppointmentsService`): a BRANCH secretary (`is_admin_hq === false` and not at the HQ branch) books with pastor-role profiles (branch_pastor/church_admin/senior_pastor) in the SAME branch; an HQ secretary (`is_admin_hq === true` or seated at the HQ branch) books with pastor-role profiles church-wide. A pastor persona creating books with secretary-role profiles (same scope). A profile reads/manages only appointments where they are `secretary_id` OR `pastor_id` (church-scoped).
  - **Schema/migration `20260830163221_add_appointments`**: `appointments` — `church_id`, `branch_id?`, `secretary_id`, `pastor_id` (free-form Profile ids, resolved to names at read), `title`, `scheduled_at`, `location?`, `notes?`, `status` (default `pending`), `created_at`/`updated_at`, `archived_at?`; indexes `[church_id, scheduled_at]`, `[secretary_id]`, `[pastor_id]`.
  - **`AppointmentsService`**: `create` (validates the viewer is secretary or pastor, fetches the counterpart by side, enforces the branch/HQ pairing scope, sets `secretary_id`/`pastor_id` by creator side, audit `CREATE` with `entity: 'appointment'`), `list` (current scope `OR secretary_id/pastor_id`, `archived_at` null default / `{ not: null }` when `archived: true`, `status`/`search`/`startDate`/`endDate` filters, pagination, plus a `summary` status breakdown), `getOne` (404 unless a party), `update` (partial fields; re-changes the counterpart within scope, audit `UPDATE`), `archive`/`restore` (audit `ARCHIVE`/`RESTORE`, wrong-state `BadRequest`), `deleteForever` (purge, must be archived, audit `DELETE`), `listContacts` (pairing picker: pastors for secretaries, secretaries for pastors, same-branch for non-HQ).
  ...
  - **DTOs** `src/appointments/dto/`: `create-appointment.dto.ts` (`title` ≤200, `scheduledAt` ISO default `pending`, `counterpartId` UUID, `location?` ≤200, `notes?` ≤2000, `status?` in `pending|confirmed|completed|cancelled`), `update-appointment.dto.ts` (`PartialType`), `list-appointments.dto.ts` (`page`/`limit` ≤200/`status`/`archived?`/`startDate`/`endDate`/`search`), `list-appointment-contacts.dto.ts` (`search`/`pastorsOnly?`/`role`/`branchId`), `appointment-response.dto.ts` (`AppointmentDto`, `AppointmentContactDto`, `AppointmentListEnvelopeDto` + `APPOINTMENT_STATUS` const).
  - **Seed** (`prisma/seeds/permissions.seed.ts`): `'appointments'` added to `RESOURCES` (25 resources / ~100 permissions). Grants: `secretary` + `branch_pastor` each get `appointments:create/read/update/delete`; `senior_pastor` and `church_admin` get full via their blocks. Existing databases need `npm run prisma:seed`.
