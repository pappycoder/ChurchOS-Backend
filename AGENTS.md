...
├── email/                  # Internal in-app email (main-role messaging)
│   ├── email.module.ts     # EmailModule
│   ├── email.service.ts    # send/list/read/trash/restore/purge/contacts
│   ├── email.controller.ts # 10 endpoints for email
│   └── dto/                # SendEmail, ListEmails, Contacts, Response DTOs
├── appointments/           # Appointment / booking registry (With pastor ↔ Who person/visitor)
│   ├── appointments.module.ts  # AppointmentsModule
│   ├── appointments.service.ts # create/list/get/update/archive/restore/purge/contacts
│   ├── appointments.controller.ts # 8 endpoints for appointments
│   └── dto/                # Create/Update/List/Contacts/Response DTOs
...
- **`removeDepartmentMember` is now idempotent — an already-removed membership returns success (204) instead of a 404.** The web department drawer's Remove button re-enables the moment the first DELETE settles (its `isPending` guard releases), while the removed row stays visible until the invalidation refetch lands — so a rapid second, identical DELETE can arrive for a membership that no longer exists. Previously the second call threw `NotFoundException`, which the drawer surfaced as a "Failed to remove member" error toast despite the first DELETE succeeding. Now, when the `departmentMember.findUnique` (still church-scoped via the department existence + archived + ownership/read-only guards, all run first) finds no record, `removeDepartmentMember` logs a `no-op removal` and returns without deleting or writing an audit row — so a retried remove is a no-op, never an error. A genuine removal still `delete`s and audit-logs `DELETE` as before; DELETE route stays `@HttpCode(NO_CONTENT)` (204).
  - **Tests**: +3 in `test/unit/admin/admin.service.spec.ts` → 92 — remove + audit, already-absent no-op (no delete, no audit), unknown department still `NotFoundException`.
  - **Verified live** (backend :3001, seeded DB): Lekki head (`branch.depthead@churchos.dev`, Youth) — add Halima Bello = **201**, DELETE #1 = **204** (audited), DELETE #2 = **204** with server log `Member 8a06beda-… not in department 4eeeb870-…: no-op removal` and no audit write (previously the second DELETE was 404).
  - Verification: `npx tsc --noEmit` clean; full suite **54 suites / 1087 tests green** (was 1084).

- **`addDepartmentMember` enforces branch membership.** The department and member selects now both include `branch_id` (admin.service.ts); when a branch-linked department (`branch_id` non-null) is being added to, the member's `branch_id` must match it or `addDepartmentMember` throws `BadRequestException("Member does not belong to this department's branch")` — branchless departments stay unrestricted and accept any church member. The rule applies uniformly to every viewer (HQ included); `removeDepartmentMember` untouched.
  - **Tests**: +3 in `test/unit/admin/admin.service.spec.ts` — cross-branch member rejected (no `departmentMember.create` call), same-branch member added, branchless department accepts a member. The existing `mockDepartment` fixture carries no `branch_id`, so the three prior add-tests are unaffected.
  - **Verified live** (backend :3001, seeded DB): Lekki head (`branch.depthead@churchos.dev`, Youth) adding HQ member Adebayo Ogundimu = **400** "Member does not belong to this department's branch"; adding Lekki member Halima Bello = **201**; HQ `church_admin` (`admin@churchos.dev`) adding a Lekki member to Choir = **400** same message; adding HQ member = **201**.
  - Verification: `npx tsc --noEmit` clean; full suite **54 suites / 1084 tests green** (was 1081); isolated `next build` for the web green.

- **Dev seed now exercises the `department_head` role end-to-end.** The two existing dept-head demo accounts were previously untestable: `org.seed.ts` created departments with no `branch_id`/`head_member_id` (both added by migration `20260908065724_department_branch_head` but never populated), and neither demo profile had a linked member — so the non-HQ head's `resolveDepartmentScope` resolved `{ headId: undefined }` → `where.head_member_id = ''` → an empty department list.
  - **`prisma/seeds/development/org.seed.ts`** — `DEPT_DEFS` extended to `[name, branch, description?, headMemberIdx?]`. Branches match each department's coordinator branch: **Choir → hq / head idx 1 (Chioma Nwosu)**, **Ushering → hq / no head**, **Media → hq / no head**, **Youth → lekki / head idx 14 (Obinna Eze)**. The department loop now also **heals existing rows** (`prisma.department.update` setting `branch_id` + `head_member_id` on the findFirst-existing path), so re-seeding an already-seeded DB repairs the branchless/headless departments a fresh seed wouldn't have. Heads were chosen as unlinked, active, same-branch members already in the department (idx 0/6/10/15 + bulk members stay profile-claimed — no `member_id` collisions).
  - **`prisma/seeds/development/users.seed.ts`** — `dept_head` (`dept.head.hq@churchos.dev`) now `firstName: 'Chioma', lastName: 'Nwosu', memberIndex: 1`; `branch_dept_head` (`branch.depthead@churchos.dev`) now `firstName: 'Obinna', lastName: 'Eze', memberIndex: 14` — mirroring the `cell_leader` prelinked-member pattern so `/profiles/me` returns a real `memberId` and the profile names match the members. Sign-in summary in `index.seed.ts` already lists both emails (no change).
  - **Verified live** (backend :3001, seeded DB): `branch.depthead@churchos.dev` (Lekki, non-HQ) → list = **only Youth**; detail Youth = 200 with `branchName: 'Lekki Campus'` + head names; PATCH own Youth = 200 with attempted `branchId`/`headMemberId` reassignment **stripped** (unchanged after); detail/patch of Choir = **404** / **403 "You can only manage your own department"**; member add to own = **201**, to another dept = **403**; create department = **403** (`departments:create` not granted). `dept.head.hq@churchos.dev` (HQ) → list = **all 4 departments** (branch names resolved); PATCH any including its headed Choir, member add, member remove = **403 "HQ department heads can view and export departments but cannot edit them"**; archive = **403** (role ceiling church_admin/senior_pastor).
  - **Correction to the earlier contract note**: the member add/remove routes carry `departments:update` (verified `admin.controller.ts` lines ~207/228) — NOT `departments:create`/`delete` — so a **non-HQ head CAN add/remove members on their own department** (ownership-guarded), while HQ heads are read-only. The web detail drawer masks Add/Remove only for HQ heads, which matches.
  - Verification: `npx tsc -p tsconfig.seed.json --noEmit` clean; seed re-run twice, idempotent (`0 created, 19 existing`); DB asserts — dept branch/head populated, profiles linked (`HQ=true` / `HQ=false`), zero head/branch mismatches.

- **`department_head` role ships end-to-end: departments carry a branch + a head, non-HQ heads manage only the department they head, HQ heads are read-only, and the seed template is reduced to the member floor + departments access.** The web counterpart (list/detail/form gates, ExportDropdown, branch + head columns, locked leader/branch form, sidebar `anyRole`) shipped this round — see web changelog. Behavior below mirrors the web masks exactly.
  - **Schema/migration `20260908065724_department_branch_head`** (applied): `departments.branch_id` (TEXT, FK → branches ON DELETE SET NULL, indexed) and `departments.head_member_id` (TEXT, nullable — stored free-form, resolved to names via the members table like cell-group leaders; no Prisma relation, `@@index`). No member-side column: `head_member_id` IS the member link.
  - **DTOs** (`src/admin/dto/`): `CreateDepartmentDto` gains optional `branchId?` (`@IsUUID`) and `headMemberId?` (`@IsUUID`); `DepartmentResponseDto` gains `branchId?`, `branchName?`, `headMemberId?`, `headFirstName?`, `headLastName?`.
  - **Scoping** (`src/common/services/branch-scope.service.ts` — new `resolveDepartmentScope`): admin-HQ holders and **everyone except a non-HQ department_head** get `{ churchOnly: true }` (departments are NOT branch-scoped for staff — being a member of a department implies church-wide read); a non-HQ `department_head` (primary `role` or in `roles[]`) resolves `{ headId: viewer.member_id }`. Key-presence detection (`'headId' in scope`) mirrors the cell-group scope pattern.
  - **`AdminService.listDepartments`**: resolves the viewer scope and, for a head scope, filters `where.head_member_id = viewer.member_id` (head never sees other departments, including archived ones — the list is active-only for them so they can't enumerate). All other callers keep church-wide access. `getDepartmentById` uses the same scope (head + Archived → 404; head + another's department → 404).
  - **Ownership + HQ read-only guards**: a non-HQ department_head can PATCH only their own department (`ForbiddenException('You can only manage your own department')`), and `branchId`/`headMemberId` are **silently stripped** from their PATCH payload (they can never reassign the department). HQ heads (any role including `department_head` with `is_admin_hq`) are **Forbidden** on all mutations — `updateDepartment`, `addDepartmentMember`, `removeDepartmentMember` (`'HQ department heads can view and export departments but cannot edit them'`). `removeDepartmentMember` also now fetches the department church-scoped (it previously deleted without loading it). Create/archive/restore/delete stay `church_admin`/`senior_pastor` — heads can neither create departments nor delete.
  - **Head-assignment validation** (new private `validateDepartmentAssignments(churchId, branchId, headMemberId)`): branch must exist in the church (else `NotFoundException`); head member must exist in the church (else `NotFoundException('Head member not found in this church')`); when both are set, the head's `branch_id` must equal the selected branch (else `BadRequestException('Head member must belong to the selected branch')`). `resolveDepartmentHeads(churchId, departments)` batch-resolves head names into the response DTOs.
  - **Routes widened** (`admin.controller.ts`, all pass `req.profile` as `viewer`): GET list + GET `:departmentId` → `@RequireRoles('church_admin','senior_pastor','branch_pastor','department_head')` + `departments:read`; PATCH + members POST + members DELETE → caller roles + `department_head` + `departments:update` / `departments:create` (member add) / `departments:delete` (member remove). No export endpoint (client-side export).
  - **Seed** (`prisma/seeds/shared/permissions.seed.ts`): the `department_head` template was broader than originally believed — trimmed to the **member floor** (`events:read, sermons:read, media:read, profiles:read, church:read`) + `departments:read` + `departments:update` (25 grants). Existing DBs: `npm run prisma:seed-perms` + `perms:*` Redis keys flushed.
  - **Tests**: `test/unit/admin/admin.controller.spec.ts` — widened-role assertions updated on GET list / GET `:departmentId` / PATCH (`hasRequireRoles` now includes `department_head`), new source-scan decorator test for GET `:departmentId`; `test/unit/common/branch-scope.service.spec.ts` +4 — `resolveDepartmentScope` church-wide (no viewer/HQ), head-scoped (primary role + roles-array detection), church-wide for other non-HQ viewers.
  - Verification: `npx tsc --noEmit` clean; full suite **54 suites / 1081 tests green** (was 1077; admin 112, branch-scope 17); isolated `next build` for the web green.

- **Hierarchical surface-permission system: every menu option now carries its own `resource:surface:action` code, with backend enforcement plus a new `view` action.** Each sidebar submenu / header child option maps to a surface whose grants are *derived* from the role's coarse grants — so every role keeps the exact menus it had before, but the API and UI now enforce at per-surface granularity. `Permission.name` stays the unique full code; `resource` stores the nested path (`events:calendar`) and `action` the final segment, so exact-string set membership works end-to-end (no schema migration).
  - **`prisma/seeds/shared/permissions.seed.ts`** — `generateAllPermissions` now emits 282 permissions: `resource:view` + coarse `resource:<action>` for all 26 resources, plus `resource:<surface>:<action>` (CRUD) for **38 surfaces** across members/attendance/giving/events/sermons/media/pastoral/visitors/assets/forms. `expandPermissions` derives each role's surface grants: holding `resource:<action>` grants the same `<action>` on every surface, and any coarse grant mints `resource:view` (so menus never change). `church:manage_staff` is gone — the roles that held it now carry `church:update`. Default matrix adds `analytics:view` for senior_pastor/branch_pastor/treasurer (the documented hybrid: per-page `/analytics/*` role ceilings stay as the tie-breaker) and branch_pastor keeps `forms:create`/`forms:update` in the RAW matrix. `super_admin` stays locked to all 282.
  - **Controllers surface-coded** — events, attendance, giving, members, sermons, media, pastoral, visitors, assets, and forms now gate endpoints with `@RequirePermissions('resource:surface:action')` (e.g. `events:tickets:create`, `attendance:records:read`, `forms:list:create`, `pastoral:notes:create`), enforced by the global `PermissionsGuard` (ALL-of, auto-passes no-decorator routes) on top of unchanged role ceilings; analytics routes use `analytics:view`. `PermissionsService` stays fully DB-driven (no ACTION_shorthand) with the per-role Redis cache (`perms:` keys, 15-min TTL) and pattern-coverage source-scan specs updated to count the surface codes.
  - **DB/code parity verified**: all 74 permission codes referenced by the web downstream (`route-permissions.ts` + sidebar + header) exist in the seeded `Permission` table.
  - Verification: `npm run prisma:seed-perms` idempotent → **9 roles / 282 permissions / 1342 mappings**; `perms:*` Redis keys flushed (clean); full suite **54 suites / 1077 tests green**; `npx tsc --noEmit` clean. Existing databases only need `npm run prisma:seed-perms` (permissions/mappings only).
...
- **Cell groups now support export-to-CSV, filtering, and search — available to every read role including the admin-HQ cell leader.** The existing `GET /admin/cell-groups` list gains `archived`, `search`, `branchId`, and `meetingDay` query filters, and a new `GET /admin/cell-groups/export` route streams the same scoped+filtered rows as a CSV (`cell-groups.csv`). This completes the admin-HQ cell leader's read/edit-only contract: they can view everything, edit details, filter, search, and export — but still cannot record attendance or add/remove members.
  ...
  - **`src/admin/admin.service.ts` — `listCellGroups`**: the `archived` boolean param is now a `ListCellGroupsDto` query object (search term is trimmed; empty blob is a no-op). New filters: `search` resolves leader-name matches first via a church-scoped `member.findMany` (`first_name`/`last_name` `contains` + `mode: 'insensitive'`), then adds `where.AND = { OR: [name contains, address contains, leader_id in resolvedIds] }`; `meetingDay` maps to `where.meeting_day`; `branchId` is honored **only** for church-only (admin-HQ / viewer-less) callers — a branch-restricted viewer is always forced to their own scoped branch. Leader-scope, archived-forcing, and branch scoping semantics are unchanged.
  - **`src/admin/admin.service.ts` — new `exportCellGroupsCsv(churchId, query, viewer?)`**: reuses `listCellGroups` (identical scope + filters), then renders CSV with columns `ID,Name,Leader,Branch,Address,Meeting Day,Meeting Time,Latitude,Longitude,Archived At,Created At`; leader/branch names resolved from the response DTOs, meets the members-export `escapeCsv` convention (quotes doubled, comma/newline wrapped).
  - **`src/admin/admin.controller.ts`**: the list route now takes `@Query() query: ListCellGroupsDto` (declarative `@ApiQuery` docs); new `GET cell-groups/export` **declared before `:groupId`** (Express literal-vs-param shadowing), `@RequireRoles('church_admin','senior_pastor','branch_pastor','department_head','cell_leader')` + `@RequirePermissions('cell_groups:read')` (same read group as the list, so anyone who can list can export) + `@Header('Content-Type','text/csv')` + `@Header('Content-Disposition','attachment; filename="cell-groups.csv"')` + `@Res()` `res.send(csv)`.
  - **Tests** (`test/unit/admin/admin.service.spec.ts` +5 → 85, `admin.controller.spec.ts`): `listCellGroups` 6 call sites updated (`false` → `{}`, `true` → `{ archived: true }`); new service tests — `branchId` honored for admin-HQ, forced-own-branch for branch_pastor, `meetingDay` filter, `search` OR across name/address/resolved-leader-ids, whitespace-search no-op, and CSV export (header row + `"Grace, Cell"` / `"12 Adeola ""Odeku"" St"` escaping + leader/branch name resolution); controller source-scan updates `cgRead` 5→6 and asserts the export route's `cell_groups:read` + widened read roles + CSV header decorator.
  - Verification: `npx tsc --noEmit`, `npm run build`, `npm run lint` clean; admin suites 112/112; full suite **54 suites / 1073 tests green** (was 1066).

- **A seeded admin-HQ cell leader demo account (`cell.leader.hq@churchos.dev`, password `ChurchOS@1234`) makes the new HQ cell-leader read/edit-only behavior exercisable.** The full seed previously only produced a non-HQ cell leader (Yemi Afolabi, Lekki), so the admin-HQ restriction added above had no login to demonstrate it with.
  - **`prisma/seeds/full/users.seed.ts`** — new `cell_leader_hq` user in the HQ staff block: `role: ['cell_leader', 'member']`, `branch: 'hq'`, `isAdminHq: true`, linked to member **Chinedu Okafor** (people.seed `EXTRA_MEMBERS[0]` → combined index 10, an HQ member no other profile claims; the existing P2002 adoption keeps this idempotent). It leads **no** cell group on purpose, so the demo exercises "see every group, edit any group, but can't record attendance or add/remove members."
  - **`prisma/seed-demo-logins.ts`** — `DEMO_ACCOUNTS` gains `cell.leader.hq@churchos.dev` (roles `['cell_leader']`, `isAdminHq: true`, RCCG church/branch) so the demo-login provisioning path mints the same account on DBs that don't run the full seed.
  - **`prisma/seed-full.ts`** — HQ sign-in email summary now lists `cell.leader.hq@churchos.dev`.
  - Verification: `npx tsc -p tsconfig.seed.json --noEmit` clean (seed file compiles); no runtime test deltas (seed-only change). Re-run `npm run prisma:seed-demo` (or `prisma:seed-demo-logins`) on a connected DB to provision the account.

- **Admin-HQ cell leaders are now read + edit-details only: they see every cell group and can edit any group's details, but cannot record attendance or add/remove members.** A viewer whose role set includes `cell_leader` **and** has `is_admin_hq: true` is deliberately removed from the membership/attendance write paths (their HQ flag already grants church-wide cell-group visibility via `resolveCellGroupScope` → `{ churchOnly: true }`, and `updateCellGroup` already lets them edit any group with no leader/branch strip). Non-HQ cell leaders are unchanged (own-group add/remove/attendance with leader/branch stripped on edits).
  - **`src/admin/admin.service.ts` — new private `assertCellLeaderHqReadOnly(viewer)`**: throws `ForbiddenException('HQ cell group leaders can view and edit cell groups but cannot record attendance or manage members')` when `cell_leader` is in the viewer's roles and `viewer.is_admin_hq`. Wired into `recordCellGroupAttendance`, `addCellGroupMember`, and `removeCellGroupMember` (after the not-found/archived checks, before the ownership logic) so an admin-HQ leader is rejected on every group, own or not. `updateCellGroup` deliberately does NOT call it — detail editing stays granted. All three write routes already forward `req.profile` as `viewer`, so no controller/role/permission/DTO/seed change was needed.
  - **Tests** (`test/unit/admin/admin.service.spec.ts`, now 80): the three previously-passing admin-HQ assertions flipped to expect `ForbiddenException` (add member, record attendance, record a walk-in — all assert no `create` fires); new "admin-HQ cell leader removing a member is Forbidden"; new "admin-HQ cell leader can still edit any group's details" asserting `branch_id`/`leader_id` are persisted (no strip).
  - Verification: `npx tsc --noEmit` + `npm run build` + `npm run lint` clean; admin.service suite **80/80**; full suite **54 suites / 1066 tests green** (was 1064).

- **Cell-group membership + attendance are now branch-scoped, and walk-ins register as real visitors in the recorder's branch.** A branch-restricted viewer (`is_admin_hq` false) can only add members / record attendance for members and visitors **from their own branch** — mirroring exactly what their member/visitor list pickers already show, so a user can never pick someone and then hit a server error. HQ viewers (`is_admin_hq` true) and viewer-less callers keep the previous church-wide behavior. Scope: **cell-group flows only** (`addCellGroupMember`, `recordCellGroupAttendance`) — main service/event attendance, DTOs, routes, and seeds untouched. No seed or schema change needed; existing DBs unaffected.
  - **`src/admin/admin.service.ts` `addCellGroupMember`: the target member's `branch_id` must satisfy `branchScope.isVisible(viewer, member.branch_id)`** for a branch-restricted viewer, else `ForbiddenException('Member … does not belong to your branch')`. Strict own-branch (no untagged-member allowance), matching `listMembers` scoping; the ownership check still runs first.
  - **`src/admin/admin.service.ts` `recordCellGroupAttendance`**:
    - **Member path** — same `isVisible` gate for branch-restricted viewers.
    - **Visitor path** — the visitor's `branch_id` must equal the viewer's OR be `null` (legacy untagged visitors stay allowed), matching `visitors.findAll` scoping, else `ForbiddenException('Visitor … does not belong to your branch')`.
    - **Walk-in path (`visitorName` only)** — a branch-restricted viewer with a branch now **creates a real `Visitor` row** pinned to `viewer.branch_id` (name split on the first space into `first_name`/`last_name`), links its `visitor_id` on the `cellGroupAttendance` row, and keeps the `visitor_name` snapshot for display — so walk-ins become follow-up targets in the recorder's branch. HQ viewers keep the free-text snapshot-only behavior (no row created). Walk-ins remain dedup-free (each registers a fresh visitor), unchanged from before.
  - **Tests** (`test/unit/admin/admin.service.spec.ts`, +10 → 78): cross-branch member add → Forbidden + no create; same-branch member add OK; admin-hq cross-branch add OK; cross-branch member/visitor attendance → Forbidden; null-branch visitor attendance allowed; walk-in creates visitor in the viewer's branch + links it on the attendance row; single-name walk-in → `last_name: null`; admin-hq walk-in stays snapshot-only. The two pre-existing branch-restricted tests' member mocks gained `branch_id` so they satisfy the new gate.
  - Verification: `npx tsc --noEmit` + `npm run build` clean; admin.service suite 78/78; full suite **54 suites / 1064 tests green** (was 1054).

- **Universal "member floor": every staff role template now grants the full member read set** (per product directive: "every role will at least have a member role in addition to whatever role they possess, and as long as they have a role with a permission available they should have access" — so a cell_leader (the reference case) sees/does everything a member does plus their own cell group, and by extension a treasurer/secretary/department_head can see/do everything a normal member can too). Access is already the union across all of a profile's roles, so the change is purely additive template grants — **profile `role` arrays are unchanged**, which keeps the web `useIsMember()` staff/member split and every server-side role check untouched.
  - **`prisma/seeds/permissions.seed.ts` — `DEFAULT_PERMISSION_MATRIX`**: `treasurer` gains the 5 member reads (`events:read`, `sermons:read`, `media:read`, `profiles:read`, `church:read` — it previously had none of them); `secretary` gains `sermons:read`, `media:read` (already had events/church/profiles reads); `department_head` gains `sermons:read`, `profiles:read`, `church:read` (already had events/media reads). Deliberately untouched: `branch_pastor` (already ⊇ member set), `senior_pastor`/`church_admin` (full), `super_admin` (locked to all), `member`, and `cell_leader` (member set + own-group `cell_groups:read/create/update` — the reference implementation). The seed's reconciliation step re-applies templates exactly, so existing DBs pick this up by re-running `npm run prisma:seed` (or `prisma:seed-perms` — permissions/mappings only, ~8 queries).
  - **`prisma/seeds/full/users.seed.ts`**: the `cell_leader` demo account now represents **Yemi Afolabi (memberIdx 15)** — who actually leads the seeded "Lekki Phase1 Fellowship" cell — so the demo exercises own-group list/detail/edit on real data. (Previously it linked Emeka Okonkwo, who leads no cell.) Names + `member_id` re-link in place via the existing idempotent upsert; no `profiles.member_id` uniqueness collision (no other profile links idx 15). The base `Emeka Okonkwo` member (idx 2) and its message/family references remain valid.
  - Verification: `npx tsc -p tsconfig.seed.json --noEmit` clean; `npm run prisma:seed-perms` + `npm run prisma:seed-demo` idempotent; smoke as `cell.leader@churchos.dev` → `/profiles/me` role `[cell_leader, member]` with `memberId` = Yemi Afolabi's, permissions = the member set + `cell_groups:read/create/update`; `GET /admin/cell-groups` returns only "Lekki Phase1 Fellowship"; PATCH own group → 200, another group → 403; `treasurer.hq@churchos.dev` now reports the 5 member reads; `npm run build` + full suite green.

- **Full seed hardened against legacy placeholder profiles (fixes `Unique constraint failed on the fields: (\`member_id\`)` when re-running `prisma:seed-demo` on a previously-seeded dev DB).** `upsertProfile` in `prisma/seeds/full/users.seed.ts` looked up an existing profile by `user_id` only, so a leftover placeholder row from an older demo seed — one that linked a member via `crypto.randomUUID()` as its `user_id` (never a real Supabase sub, e.g. the `seedProfiles` church_admin row) — was invisible to the lookup and `create()` then violated the unique `profiles.member_id` constraint (observed collision: the seed's `member.hq` profile vs the placeholder church_admin sharing Adebayo Ogundimu's member). The create path now catches P2002, resolves the conflicting row **by `member_id`**, and **adopts it in place** (re-points the row's `user_id`/role/branch/email to the real auth user's data) instead of failing. Conflict detection checks `meta.target` **and falls back on the error message** because the PrismaPg driver adapter surfaces P2002 with `meta = { modelName, driverAdapterError }` (no `target`). Non-member-claiming profiles and non-P2002 errors are rethrown unchanged.
  - Verification: `npx tsc -p tsconfig.seed.json --noEmit` clean; `npm run prisma:seed-demo` completes to "🎉 FULL seed completed successfully!" with **18 profiles** (member.hq = Adebayo Ogundimu adopted from the placeholder row, cell.leader = Emeka Okonkwo, bulk members 3/5/7/8/9 linked) and re-runs idempotently (second pass: 0 created / 18 existing).

- **Cell-leader detail-page blocker fixed end-to-end: cell-group scope discriminated by key presence + `/profiles/me` emits `memberId`.** Root cause: `request-context.middleware.ts` maps a NULL `profiles.member_id` to `undefined`, so a cell leader with no linked member resolved to `resolveCellGroupScope` → `{ churchOnly: false, leaderId: undefined }` with **no `branchId`**. The old `scope.leaderId !== undefined` checks misread that as a branch scope → `listCellGroups` fell through to an undefined `scope.branchId` (no filter → **leaked every church group**) and `getCellGroupById`'s branch-fallback visibility check had no branchId → **404 on every group** (list OK, detail 404).
  - **`src/admin/admin.service.ts`** — `listCellGroups` and `getCellGroupById` now detect a leader scope by **key presence** (`isCellLeaderScope = !scope.churchOnly && 'leaderId' in scope`) and never fall through to the branch filter for it: list sets `where.leader_id = scope.leaderId || ''` (a null/empty member id matches zero groups — the honest answer); detail visibility is `isCellLeaderScope ? (!!scope.leaderId && group.leader_id === scope.leaderId) : (!!scope.branchId && group.branch_id === scope.branchId)`.
  - **`src/profile/dto/profile-response.dto.ts` + `src/profile/profile.service.ts`** — `ProfileResponseDto` gains top-level `memberId?: string`; `mapToResponseDto` now emits `profile.member_id || undefined` (the scalar is selected on every profile query, so `/profiles/me` and list/detail responses all carry it). The web owns-gate (`profile.memberId === group.leaderId`) is now satisfiable client-side for linked leaders.
  - **Tests** (`test/unit/admin/admin.service.spec.ts` +4, `test/unit/profile/profile.service.spec.ts` +2): null-member leader list-scopes to zero groups; owning-leader detail fetch OK / non-owning leader 404 / null-member leader 404; `/profiles/me` emits the linked member id and leaves it undefined when unlinked.
  - Verification: `npx tsc --noEmit` clean, `npm run build` clean, admin suite **93** + profile suite **58** tests green; backend restarted (pid 700797, :3001, detached via `setsid`). Live as `cell.leader@churchos.dev`: `/profiles/me` `memberId` = `b38126fb-e922-4209-b4a0-ba0c37d4e0e2` = Victory Cell Group's `leaderId`, detail `GET /admin/cell-groups/dc21a66f-…` → 200. (Operational: the demo profile's `member_id` was linked via SQL in the previous round; the demo-logins seed keeps `member_id: null` deliberately — the null-member boundary is now covered by tests.)

- **Cell group leaders can manage their own group (backend enforcement).** Completes the "cell leader manages only their own cell group" scope: the API now lets a `cell_leader` edit their own group's details (fields except `branch` + `leader`), add/remove its members, and (already allowed) record attendance — while archive/restore/delete stay pastor-only and everything is hard-enforced server-side, matching the web gates.
  - **`src/admin/admin.controller.ts`** — `'cell_leader'` added to `@RequireRoles` on `GET cell-groups/nearest` (no behavioral change — leaders are expected to spot nearby groups) and on the three management routes: `PATCH cell-groups/:groupId`, `POST cell-groups/:groupId/members`, `DELETE cell-groups/:groupId/members/:memberId`. These three now pass `req.profile` through as `viewer` to the service so it can enforce ownership.
  - **`src/admin/admin.service.ts`** — new private `assertCellLeaderOwnership(viewer, group)`: when the viewer's role set includes `cell_leader` and they are not `is_admin_hq`, the viewer must be the group's `leader_id` (compared via `member_id`), else `ForbiddenException('You can only manage your own cell group')` — the same rule `recordCellGroupAttendance` already used. Applied in:
    - `updateCellGroup` (new optional 5th `viewer?: ViewerScope | null`): after the existence/archived checks, enforces ownership and, for an owned non-HQ cell leader, **strips `branch_id` + `leader_id` from the update payload** (`restrictedLeader`) so a leader can never reassign the group to another branch or hand leadership off — the other fields (name, address, meeting day/time, coords) update normally.
    - `addCellGroupMember` (viewer param): ownership enforced before inserting any member.
    - `removeCellGroupMember` (viewer param): now also fetches the group church-scoped (previously it deleted without ever loading the group) and enforces ownership + archived check + 404-if-missing, fixing the pre-existing gap where any caller with the route permission could remove anyone.
  - **`prisma/seeds/permissions.seed.ts`** — `cell_leader` template gains `'cell_groups:update'` (7 → 8 grants = the full member set + `cell_groups:read/create/update`) so the PATCH route's `cell_groups:update` permission check passes and the web Edit/Remove-Member affordances activate. Description updated. Reseed with `npm run prisma:seed-perms`.
  - **Tests** (`test/unit/admin/admin.controller.spec.ts` + `admin.service.spec.ts`): controller specs now assert `'cell_leader'` in the role lists for nearest/PATCH and add role assertions to the members POST/DELETE specs; service specs add a `mockCellGroupMember` fixture + 7 new cases — forbid a non-own group update, allow an own-group update while stripping leader/branch, allow own-group member add, forbid non-own add, allow own-group remove, forbid non-own remove, and forbid a cell leader whose profile has no linked member.
  - Verification: `npx tsc --noEmit` clean, `npm run build` clean, admin suite **89 tests green**, full suite green.

- **Events registrations end-to-end staff-only + archived cell groups invisible to cell leaders** (completes the cell_leader member-parity enforcement on the backend API; the earlier round shrank the `cell_leader` permission template, this round closes the two remaining API-only gaps so the web members-block can't be bypassed by hitting the endpoints directly).
  - **`src/events/events.controller.ts`**: `GET /events` (`listEvents`, line 96) and `GET /events/:eventId/registrations` (`listRegistrations`, line 461) — previously **auth-only** (no `@RequirePermissions`/`@RequireRoles`), so any authenticated user (including member and cell_leader) could list every church event and every registration with payment details. Both now carry `@UseGuards(RolesGuard)` + `@RequireRoles('church_admin', 'branch_pastor', 'secretary')` (the same staff set the existing ticket-validation route uses; there is no `cell_admin` role in this codebase). Cell leaders and members are now staff-blocked at the API, matching the web `useIsMember()` Access-denied guards. `GET /events/:eventId` (`getEvent`) intentionally stays open so members/cell leaders can still view a single event for ticketing — stateful with the web `[eventId]` detail page.
  - **`src/admin/admin.service.ts` `listCellGroups`**: when the caller resolves to a cell-leader scope (`resolveCellGroupScope` → `{ churchOnly: false, leaderId }`), the `archived` param is **forced to active-only** (`archived_at: null`) regardless of what the caller passed — a silent filter, never a 403 — so leaders can't widen their view to archived groups via `?archived=true`. Admin-HQ and branch-restricted viewers are unaffected.
  - **`src/admin/admin.service.ts` `getCellGroupById`**: for a cell-leader scope, an archived group now resolves to `NotFoundException` (silent 404), consistent with the active-only list filter — leaders get the same "doesn't exist" answer whether they reach an archived group through list or by ID.
  - **Web counterpart** (`ChurchOS-Web` cell-groups list page): the Active|Archived segmented filter is hidden from `isCellLeader` users (they also already had no archive/restore affordances — `cell_groups:update` is not in their 7 grants), so the UI can't even request the archived view.
  - Verification: `npx tsc --noEmit` clean (both repos), `npm run build` clean (both), full suite **54 suites / 1041 tests green** (unchanged — backend service/controller edits are covered by the source-scan controller specs which assert role decorators, and the seed logic is untouched).

- **`cell_leader` permissions reduced to member + cell-group access only** (per product directive: "a cell leader sees everything a member sees and can do; the only additional thing is cell group access"). All members can already hold other roles (roles are arrays), so no multi-role change was needed.
  - **`prisma/seeds/permissions.seed.ts` — `cell_leader` template** cut from 11 grants to **7** = the full `member` set (`events:read`, `sermons:read`, `media:read`, `profiles:read`, `church:read`) + `cell_groups:read` + `cell_groups:create` (attendance recording for their own group; `POST cell-groups` creation stays role-capped to pastors). Dropped: `assets:create/read/update`, `visitors:read/create`, `members:read`, `giving:read`, `emails:read`. Role description updated to "member-level access plus attendance recording for their own group".
  - **`seedPermissions` is now reconciling** (`prisma/seeds/permissions.seed.ts`): the prior additive-only `createMany({ skipDuplicates: true })` meant permissions removed from a template lingered forever. New step 4 runs a per-role `deleteMany({ where: { role_id, permission_id: { notIn: desiredIds } } })` for every non-`super_admin` null-church template role, so template grants always EXACTLY match `DEFAULT_PERMISSION_MATRIX`. `super_admin` stays locked to all; per-church custom roles are never reconciled (addressed by role id from the null-church lookup only). This also self-heals prior drift — first run removed 8 stale `cell_leader` + 3 stale `member` mappings (legacy `members:read`/`giving:read`/`families:read` remnants).
  - **`src/assets/assets.controller.ts`**: `'cell_leader'` removed from `WRITE_ROLES` (template permission drop already 403s all asset endpoints — this is consistency cleanup) and the header comment no longer lists it.
  - **`src/email/email.service.ts`**: `'cell_leader'` removed from `MAIN_ROLES` — leaders can no longer be email recipients/contacts, matching member parity (decision: strip entirely). `email.service.spec.ts` updated: `MAIN_ROLES` now asserts `not.toContain('cell_leader')`.
  - **Unchanged (intended, verified)**: events `isStaff` already treats `cell_leader` as a member for tickets (`events.controller.ts:139,529`); `admin.controller.ts` cell-group route role caps + `admin.service.ts`/`branch-scope.service.ts` own-group ownership enforcement remain — that IS the "only additional access"; profile role DTOs keep `cell_leader` assignable (multi-role).
  - **Operational**: re-ran `npm run prisma:seed-perms` on the local DB — reconciliation removed the stale grants; verified cell_leader = exactly `cell_groups:create/read`, `church:read`, `events:read`, `media:read`, `profiles:read`, `sermons:read`. Existing databases require re-running `npm run prisma:seed-perms` (or `prisma:seed`) to shrink the grants.
  - Verification: `npx tsc --noEmit` clean, `npm run build` clean, full suite **54 suites / 1041 tests green**.

- **Demo logins now resolve to real `profiles` rows (fixes 404 "User profile not found" on every demo login).** The plain seed writes profiles with `user_id: crypto.randomUUID()` — IDs that never match a real Supabase auth sub — so every remote demo account (`cell.leader@churchos.dev`, `admin@churchos.dev`, …) authenticated fine but `GET /profiles/me` 404'd, and every downstream route read `req.profile` = `undefined` (notifications 500 `church_id` of undefined, email 403s, audit-log `audit_logs_church_id_fkey` failures on login/logout).
  - **New `npm run prisma:seed-demo-logins`** (`prisma/seed-demo-logins.ts`): resolves each demo account's auth **sub by email** via the Supabase admin API (`admin.listUsers`), then `profile.upsert`s a row keyed to that real `user_id`. Idempotent, touches only `profiles`. `tsconfig.seed.json` already includes `prisma/**/*.ts`.
  - **Provisioned accounts** (`DEMO_ACCOUNTS`, easily extended): `admin@churchos.dev` → `['church_admin']` (RCCG church `d922c528…`, branch `c7171ba1…`, `is_admin_hq: true`) and `cell.leader@churchos.dev` → `['cell_leader']` (same church/branch, `member_id: null` so the "no linked member" ticket-list fix is exercised — claim auto-links via `ensureMemberId`).
  - Verification: `npx tsc -p tsconfig.seed.json --noEmit` clean; run created both profiles (subs `f4dd6661-…` / `bc69c5b4-…`), `member_id` NULL on cell leader.

- **`listAllTickets` no longer leaks the whole church's tickets to a non-staff caller with no linked Member.** Root cause of the "cell leader saw everyone's tickets / never his own" report: the controller spread `memberId: req?.profile?.member_id` for non-staff, and the service only applied the filter `if (memberId)` — so a null/undefined `member_id` silently skipped the filter and returned **every** church ticket. Now an explicit empty `member_id` scopes to **zero rows** (the honest answer — that caller holds no tickets), while the un-keyed staff path stays church-wide.
  - **`src/events/events.controller.ts` — `listAllTickets`**: non-staff branch now passes `memberId: req?.profile?.member_id ?? ''` so the identity is always sent (possibly empty), never `undefined`.
  - **`src/events/events.service.ts` — `listAllTickets`**: guard changed from `if (memberId)` to `if (memberId !== undefined)` with `where.member_id = memberId || ''` — an empty value matches no rows (tickets `member_id` is TEXT), so a missing identity can never widen the query. Staff calls (no `memberId` key) remain church-wide.
  - Claim is unaffected — `createTicket` still auto-creates + links a Member via `ensureMemberId` on self-claim, so a leader's next list is non-empty.
  - **Tests**: `test/unit/events/events.service.spec.ts` — `listAllTickets` gains "scope to no rows (never all tickets) when a non-staff caller has no linked member" (`memberId: ''` → `where.member_id === ''`, church filter intact); the existing church-wide (no key) and self-scope specs are unchanged. Events suite **48 tests green**; `npx tsc --noEmit` clean; `npm run build` clean.
...
- **Cell leader ticket flow now identical to member.** The prior round's `listAllTickets` branch for cell_leader was a custom `branchId` filter (church-wide events excluded) and `createTicket` used `strictBranch: true`. Both were wrong: cell leaders should see and claim exactly what members see. The `isStaff` guard now excludes both `member` and `cell_leader`, so cell leaders enter the same default (member) branch on both endpoints.
  - **`src/events/events.controller.ts` — `listAllTickets`**: `isStaff` now checks `roles.some(r => r !== 'member' && r !== 'cell_leader')`; cell_leader enters the default member branch and the handler scopes the result to `memberId: req.profile?.member_id`. Removed the `isCellLeader`/`isAdminHq`/`branchScope`/`branchId` spread that previously set `branchId: req.profile?.branch_id`.
  - **`src/events/events.controller.ts` — `createTicket`**: same `isStaff` change; removed the `cellLeaderSelfClaim` (`isCellLeader && !isAdminHq`) + `strictBranch` viewer props. Cell leaders now flow through the plain member path (self-claim, own-branch events + church-wide/null-branch events).
  - **`src/events/events.service.ts` — `listAllTickets`**: removed `branchId` from the filters type and `where.event` — the cell_leader branch is no longer needed.
  - **`src/events/events.service.ts` — `createTicket`**: removed `strictBranch` from the viewer type; restored the member-scope branch check (`event.branch_id && event.branch_id !== viewer.branchId`).
  - **`test/unit/events/events.service.spec.ts`**: replaced the four strict-branch cell-leader tests with member-identical tests (own-branch claimable, null-branch church-wide claimable, other-branch rejected); removed all `branchId`-based `listAllTickets` tests. Self-scope tests added for both `cell_leader` and `member` roles.
  - **Reseed (operational)**: `npm run prisma:seed-perms` re-applied cell_leader grants on local DB; `perms:*` Redis cache flushed (was already clean/empty). Cell_leader confirmed with 11 grants (`cell_groups:read/create`, `cell_groups:members:read/write`, `events:read`, `visitors:read/create`, `profiles:read`, `church:read`, `branches:read`).
  - **Tests**: 54 suites / 1040 tests all green; `npx tsc --noEmit` clean; `npm run build` clean.
...
- **Cell group leader fixes: branch-scoped visitors + cell-group attendance ownership + branch-wide tickets.**
  ...
  - **Seed grants** (`prisma/seeds/permissions.seed.ts`): `cell_leader` template += `cell_groups:create`, `events:read`, `visitors:read`, `visitors:create`; description updated to "Cell group leader who records attendance for their own group and follows up on visitors". Re-seed needed on existing DBs (`npm run prisma:seed`).
  - **Visitors branch derivation** (`src/visitors/*`): `create(dto, churchId, userId, viewer?)` now resolves `branch_id` — non-HQ viewers are pinned to their own branch (ignore `dto.branchId`); HQ/unspecified creators honor `dto.branchId ?? null` with explicit-branch validation against the church's branches (`BadRequestException('Branch not found')`). `findAll(..., viewer?)` scopes non-HQ viewers to `OR: [{ branch_id: viewer.branch_id }, { branch_id: null }]` (legacy untagged visitors stay visible). `branchId`/`branchName` emitted on responses; `CreateVisitorDto.branchId` optional; `visitors.controller.ts` passes `req.profile` to create/findAll.
  - **Cell group attendance ownership** (`src/admin/admin.service.ts` + `admin.controller.ts`): `POST admin/cell-groups/:groupId/attendance` `@RequireRoles` adds `cell_leader`; `recordCellGroupAttendance(..., viewer?)` rejects non-HQ cell leaders who aren't the group's leader (`viewer.member_id` + `group.leader_id === viewer.member_id` else `ForbiddenException('You can only record attendance for your own cell group')`). admin-hq cell leaders unaffected.
  - **Tickets three-way role class** (`src/events/events.controller.ts` + `events.service.ts`): `listAllTickets` — cell_leader (non-HQ) → branch-wide list via new `branchId` filter (`where.event = { church_id, branch_id }`, church-wide/null-branch events excluded); staff → church-wide; default → self `member_id`. `createTicket` — cell_leader (non-HQ) self-claims via the member path plus `strictBranch: true`, enforcing `event.branch_id === viewer.branchId` (church-wide events not claimable by cell leaders); full staff path unchanged (`events:create`).
  - **Tests**: visitors service spec now 42 tests; admin service spec + controller spec 82 (4 new owner-enforcement tests); events service spec gains `listAllTickets` (4) + strict-branch cell-leader tests (4). Full suite **54 suites / 1042 tests green**; `npx tsc --noEmit` clean; `npm run build` + `npm run lint` clean.

- **Avatar URLs added to appointment and email response DTOs.** Profile avatar images now flow through the API so the frontend can render them instead of placeholder initials.
  - **`src/appointments/dto/appointment-response.dto.ts`** — `AppointmentDto` gains `pastorAvatarUrl?: string` and `personAvatarUrl?: string` (`@ApiPropertyOptional`).
  - **`src/appointments/appointments.service.ts`** — `buildDetail()` profile `select` now includes `avatar_url: true`; the returned DTO maps `pastor?.avatar_url` → `pastorAvatarUrl` and `person?.avatar_url` → `personAvatarUrl` (visitors always emit `undefined` since they have no Profile avatar).
  - **`src/email/dto/email-response.dto.ts`** — `EmailItemDto` and `EmailDetailDto` each gain `senderAvatarUrl?: string` (`@ApiPropertyOptional`).
  - **`src/email/email.service.ts`** — new private `resolveSenderAvatars(churchId, senderIds)` batch-resolves `profile.avatar_url` into a `Map<string, string | undefined>`. Wired into: (1) **inbox list** — avatar map resolved for all unique sender IDs after `fetchMessages`, passed to `mapInboxItem`; (2) **sent list** — sender avatar resolved for the current user, passed to `mapSentItem`; (3) **`buildDetail`** — the denormalized `sender_name` path now also queries `avatar_url` from the Profile; the fallback Profile lookup path adds `avatar_url: true` to the select. `mapInboxItem` and `mapSentItem` signatures accept optional `senderAvatarUrl` and include it in the returned DTO.
  - Web counterpart (types + avatar rendering) shipped same round — see web changelog.
  - Verification: `npx tsc --noEmit` clean; `npm run test` — **54 suites / 1023 tests green**; `npm run build` + `npm run lint` clean.

- **Member ticket self-claim no longer requires a linked Member record.** Previously a member could only claim a ticket if their profile already had `member_id` set — the member claim endpoint demanded `dto.memberId` (controller) and `createTicket` threw `ForbiddenException('No member profile is linked...')` when `req.profile.member_id` was empty. Since `/profiles/me` never surfaces a member via `getMyProfile`, members without a pre-linked Member could never claim. The backend now resolves the caller's own member identity on the fly:
  - **`src/events/events.service.ts` — new private `ensureMemberId(userId)`**: fetches the profile (`id, member_id, first_name, last_name, church_id, branch_id`); returns the linked `member_id` if set, otherwise `member.create({ church_id, branch_id, first_name, last_name, status: 'active' })` + `profile.update({ member_id })` and returns the new id. Mirrors the existing `SermonsService`/`PastoralService` convention exactly.
  - **`createTicket` `enforceSelf` branch**: when `viewer.memberId` is absent, it now resolves via `await ensureMemberId(userId)` (keeping the same `ForbiddenException` only if resolution still yields nothing); `memberId` defaults to the resolved self id when the caller omits it, and the self-only check asserts `memberId === resolvedSelfId`. `visitorId` rejection and branch scope are unchanged.
  - **`src/events/events.controller.ts` `POST :eventId/tickets`** member branch: no longer throws `BadRequestException('Members must provide a memberId...')` when `dto.memberId` is omitted — `memberId` is optional for members and the service fills in the caller's own id. (CreateTicketDto already declared `memberId?` optional.)
  - **Tests**: `test/unit/events/events.service.spec.ts` — new `createTicket (member self-claim)` block (3 tests): resolves + auto-links via `ensureMemberId` when the profile has none, rejects claiming for a different memberId, rejects visitor assignment in the self-claim path.
  - Web counterpart (claim dialog no longer gates on `memberId`) shipped this round — see web changelog.
  - Verification: `npx tsc --noEmit` clean; `npm run test` — **54 suites / 1023 tests green** (was 1020; events spec 41). Lint unchanged apart from the pre-existing `media.seed.ts` unused-`Prisma` error.

- **Member role gated down to core read-only scope (backend seed).** Web sidebar/media/docs gating shipped this round (see web changelog); the backend enforcement counterpart removes the member-role grants that the web sidebar was hiding, so defense-in-depth matches the UI:
  - **`prisma/seeds/permissions.seed.ts` member template** — removed `members:read`, `giving:read`, `families:read`. Members keep only `events:read`, `sermons:read`, `media:read`, `profiles:read`, `church:read` (self-service profile via `/profiles/me` needs `profiles:read` only, not `members:read`). The `member` seed lines are the ONLY change — `cell_leader` (which legitimately holds `members:read`/`giving:read` for its cell-group management) and every other template is untouched.
  - **Note**: members list/detail GET endpoints are auth-only (no `@RequirePermissions`/`@RequireRoles`) — this change only stops the member role from *reporting* those permissions via `/profiles/me`, which is what drives the web sidebar/media/docs hiding. Existing databases must re-run `npm run prisma:seed` to pick up the reduced member grants.
  - Verification: `npx tsc --noEmit` + `npm run test` green (expect 54 suites / 1020 tests unchanged, seed-only).

- **Event tickets: members can self-assign one ticket for their branch's events; ticket list self-scopes for non-staff.**
  - **`src/events/events.controller.ts` `POST :eventId/tickets`** — the admin-only decorators (`@RequireRoles('church_admin','branch_pastor')` + `@RequirePermissions('events:create')`) are removed (kept `@UseGuards(RolesGuard)` + `@HttpCode(201)`; class-level `JwtAuthGuard` unchanged). The handler branches on the caller: **staff** (any role != `member`) still must hold `events:create` else `ForbiddenException('You do not have permission to create tickets')`; **members** may claim a ticket for themselves via the new viewer path (`memberId: req.profile?.member_id`, `branchId: req.profile?.branch_id`, `isAdminHq`, `enforceSelf: true`). `GET /events/management/tickets` (`listAllTickets`) now passes `...(isStaff ? {} : { memberId: req?.profile?.member_id })` so non-staff callers are scoped to their own tickets.
  - **`src/events/events.service.ts` `createTicket(..., viewer?)`** — added an optional viewer context. When `viewer.enforceSelf`: requires a `viewer.memberId` (`ForbiddenException('You do not have permission to claim a ticket')`), requires `memberId === viewer.memberId` else `ForbiddenException('Members can only claim a ticket for themselves')`, rejects `visitorId` (`ForbiddenException('Members cannot create a ticket for a visitor')`), and rejects when `!viewer.isAdminHq && event.branch_id && event.branch_id !== viewer.branchId` (`ForbiddenException('This event belongs to another branch')`) — church-wide events (null branch) remain claimable by any member. The existing one-ticket-per-event guard (status notIn `cancelled`/`refunded`) is unchanged.
  - **`src/events/events.service.ts` `listAllTickets`** — filters signature gains optional `memberId`; when set, `where.member_id = memberId` self-scopes the result (staff pass no `memberId` and see everything).
  - Web counterpart (member claim dialog + Assigned-Tickets-only management page + sidebar avatar fix) shipped this round — see web changelog.
  - Verification: `npx tsc --noEmit` clean; `npm run test` green (54 suites / 1020 tests). (`npm run lint` still reports one pre-existing unrelated error in `prisma/seeds/full/media.seed.ts`.)

- **Fault-tolerance hardening for external integrations (Redis, Resend, Termii, Paystack, Flutterwave, Supabase).** Outages now degrade gracefully and notify operators (in-app bell) instead of crashing requests or leaving the system in an inconsistent state. Four code gaps closed + a new alerting path:
  - **Step D — in-app integration alerts** (new `src/notifications/integration-alert.service.ts`): `IntegrationAlertService.notify(churchId, integration, title, message, data?)` raises a `system` notification to the church's `super_admin` + `church_admin` profiles (`role: { hasSome: [...] }`, `prisma/schema.prisma` line ~207), merging `data` under `{ integration, ...data }`. **Fire-and-forget by design**: every path swallows errors and the per-recipient `createNotification` failures are caught, so alerting can never break the failing request. Registered + exported from `NotificationsModule`, token `INTEGRATION_ALERT_SERVICE_TOKEN` added to `notification-tokens.ts`, exported via `notifications/index.ts`. Alert wiring: `ResendService.sendEmail`, `TermiiService.sendSms` and `WhatsAppService.sendMessage`/`sendTemplateMessage` call it on not-configured/HTTP-error/catch paths. Because `NotificationsModule` imports `CommunicationModule`/`WhatsAppModule` (a cycle back to the alert service), those component services resolve it **lazily via `ModuleRef.get(INTEGRATION_ALERT_SERVICE_TOKEN, { strict: false })`** — the same pattern `AuditLoggingService` uses for `NOTIFICATIONS_SERVICE_TOKEN`. Redis outages remain **log-only** (relied on `/health` `degraded`) to avoid alert storms; alerts fire only for Resend/Termii/Paystack/Flutterwave gateway failures.
  - **Step A — auth 2FA login catch-all split** (`src/auth/auth.service.ts` `login()`): the single `try` that staged the pending session AND emailed the OTP (which issued a full session on ANY error — including silently skipping 2FA when Resend was down) is now two independent blocks. A **Redis outage during staging fails open** (returns the session directly — can't stage/verify a code). An **email-send failure fails closed**: the pending Redis key is deleted best-effort and a clean `ServiceUnavailableException('Could not send your verification code. Please try again.')` is thrown so the caller can resend. `sendLoginCode`'s send trigger is `ResendService.sendEmail`, which fires its own Step-D alert.
  - **Step B — no more orphaned `pending` transactions** (`src/giving/giving.service.ts` `initializePayment`): `provider.initializeTransaction(...)` is now wrapped. On gateway failure the just-created pending transaction is marked `failed` (best-effort, logged on failure), a `ServiceUnavailableException` is raised, and a Step-D alert fires from `GivingService` (which has `churchId`) — Paystack/Flutterwave constructors stay unchanged. Provider `BadRequestException`/`ConflictException`/`ServiceUnavailableException` are rethrown as-is.
  - **Step C — clean gateway fetch errors** (`src/giving/services/paystack.service.ts` + `flutterwave.service.ts`): added a private `request<T>()` helper wrapping `fetch` + JSON parsing that throws `ServiceUnavailableException` on network/DNS/timeout/invalid-response failures; all `initializeTransaction`/`verifyTransaction`/`chargeAuthorization` fetch calls go through it. HTTP-level provider errors (e.g. `status: 'error'` bodies) remain `BadRequestException` so the UI keeps its validation messages.
  - **Tests**: new `test/unit/notifications/integration-alert.service.spec.ts` (7 tests: admin targeting, per-admin fan-out + payload merge, no-admin no-op, per-recipient failure swallow, admin-query failure swallow, empty-churchId no-op); `auth.service.spec.ts` +2 (2FA fail-open on Redis outage with no email sent, fail-closed on email failure with pending deletion + `ServiceUnavailableException`); `giving.service.spec.ts` +2 (gateway-init failure marks tx `failed` + fires alert + rethrows `ServiceUnavailableException`; provider `ServiceUnavailableException` rethrown un-wrapped); `flutterwave.service.spec.ts` +2 (init/verify network-failure → `ServiceUnavailableException`); `WhatsAppService` spec updated for its new 5th `ModuleRef` ctor arg; giving/recurring specs updated for the new `IntegrationAlertService` ctor arg. Full suite: **54 suites / 1020 tests green** (was 53/1008); `npm run build`, `npm run lint`, `npx tsc --noEmit` all clean.
...
- **Appointment contacts picker returned 0 for a null-branch church_admin (fix).** The logged-in church_admin dev profile (`is_admin_hq = false`, `branch_id = null`) was treated as a non-HQ booker by `AppointmentsService.listContacts`, so the branch-scope filter collapsed to `profile.branch_id = ''` and matched nothing → the With/Who pickers rendered empty (0 contacts). This is a **data + seed** fix, not a service-logic change:
  ...
  - **Seed (`prisma/seeds/profiles.seed.ts`)**: the church_admin `profile.create` now sets `is_admin_hq: true`, matching the documented auth-service rule ("church_admin defaults `is_admin_hq` on") so a fresh seed can't reproduce the empty picker. The `findFirst` idempotency/skip path is unchanged.
  - Note: the second `Adebayo Ogundimu` church_admin (`4f65e21c…`) belongs to a **different church** — not a true duplicate.
  - Tests: no service change, so no new specs; appointments suite still **35 tests green**; `npm run build` + `npm run lint` clean.

- **Appointment/booking registry `src/appointments/` — With/Who model at `/api/v1/appointments`.** A simple CRUD registry (a record: title, scheduled date/time, With pastor, Who person/visitor, location, notes, status) — not a threaded request. Permission-gated for secretary + pastor roles (`appointments:create/read/update/delete`) with `super_admin` locked to all. **With/Who model**: every appointment has a fixed **With** party (a pastor: branch_pastor/church_admin/senior_pastor) and a **Who** party (a staff/member profile **or** an existing visitor). The **booker** is implicit (no stored organizer column) and may be a secretary, any pastor role, or super_admin; new visitors can only be created at **create** time (edit picks existing visitors only). **Scope** (from `AppointmentsService`): a BRANCH booker (`is_admin_hq === false` and not at the HQ branch) picks With/Who participants in the SAME branch; an HQ booker (`is_admin_hq === true` or seated at the HQ branch) picks participants church-wide. A profile reads/manages only appointments where they are `person_id` OR `pastor_id` (church-scoped).
  - **Schema/migration `20260830181252_add_appointment_visitor_person`** (applied on top of `20260830163221_add_appointments`): `appointments.secretary_id` **renamed** `person_id` (data preserved), added nullable `visitor_id`; indexes updated to `[person_id]`, `[pastor_id]`, new `[visitor_id]`. `pastor_id` = With, `person_id` = Who profile (or the booker's profile id when Who is a visitor, so booker-scoped reads still resolve), `visitor_id` = non-null only when Who is a visitor.
  - **`AppointmentsService`**: `create` (eligibility `isSecretary || isPastor(any PASTOR_ROLE) || isSuperAdmin`; resolves With via `fetchProfileInScope` requiring a PASTOR_ROLE and asserts branch scope, resolves Who as exactly one of a profile (`whoId`) or an existing visitor (`whoKind:'visitor'`+`visitorId`); sets `pastor_id`, `person_id`, `visitor_id`, `branch_id` = With pastor's branch; self-same + mutual-exclusion guards; audit `CREATE`), `list` (scope `OR person_id/pastor_id`, archived/status/search/date filters, pagination, `summary`), `getOne` (404 unless a party), `update` (re-resolves only the changed With/Who/visitor fields, conditional so plain field edits skip participant fetch; no inline new-visitor; audit `UPDATE`), `archive`/`restore` (audit `ARCHIVE`/`RESTORE`, wrong-state `BadRequest`), `deleteForever` (purge, must be archived, audit `DELETE`), `listContacts` (`kind` param: `with` → PASTOR_ROLES profiles, `who` → all profiles plus `includeVisitors` → existing visitors; same-branch for non-HQ / church-wide for HQ).
  - **`AppointmentsController`** (`@Controller('appointments')`, `JwtAuthGuard`-only, permission-gated, no role ceilings): `GET /appointments/contacts` (`appointments:read`, declared before the param route), `GET /appointments` (list, `appointments:read`), `POST /appointments` (`appointments:create`, `@HttpCode(201)`), `GET /appointments/:appointmentId` (`appointments:read`), `PATCH /appointments/:appointmentId` (`appointments:update`), `POST /appointments/:appointmentId/archive|restore` (`appointments:update`), `DELETE /appointments/:appointmentId` (purge, `appointments:delete`). Registered via `AppointmentsModule` (imports `AuthModule`) in `app.module.ts`.
  - **DTOs** `src/appointments/dto/`: `create-appointment.dto.ts` (`title` ≤200, `scheduledAt` ISO, `withId` UUID (With pastor), `whoId?` UUID + `whoKind?` (`profile|visitor`, default profile) + `visitorId?` UUID (Who, exactly one source), `location?` ≤200, `notes?` ≤2000, `status?` in `pending|confirmed|completed|cancelled`), `update-appointment.dto.ts` (`PartialType`), `list-appointments.dto.ts` (`page`/`limit` ≤200/`status`/`archived?`/`startDate`/`endDate`/`search`), `list-appointment-contacts.dto.ts` (`kind` `with|who`, `search`/`includeVisitors?`/`role`/`branchId`), `appointment-response.dto.ts` (`AppointmentDto` with `pastorId/pastorName/pastorRole` + `personId/personName/whoKind/visitorId?/visitorName?`, `AppointmentContactDto` with `kind`, `AppointmentListEnvelopeDto` + `APPOINTMENT_STATUS` const).
  - **Seed** (`prisma/seeds/permissions.seed.ts`): `'appointments'` added to `RESOURCES` (25 resources / ~100 permissions). Grants: `secretary` + `branch_pastor` each get `appointments:create/read/update/delete`; `senior_pastor` and `church_admin` get full via their blocks. Existing databases need `npm run prisma:seed`. `prisma/seeds/profiles.seed.ts` now also seeds a **secretary** profile (`Grace Okafor`, same branch as the admin) so the pairing picker has a secretary counterpart to book against in dev (returns `secretaryProfileId`).
  - **Tests**: new `test/unit/appointments/appointments.service.spec.ts` (Proxy Prisma mock, 21 tests: create scope validation incl. cross-branch rejection + HQ church-wide + non-role/self rejection, list filters + summary, getOne party/404, update fields + pastor re-change, archive/restore/purge lifecycle, contacts side/branch scoping) + `test/unit/appointments/appointments.controller.spec.ts` (source-scan, 10 tests: every route's `@RequirePermissions` + route-order guard). Full suite: **53 suites / 1003 tests green** (was 51/973); `npm run build`, `npm run lint`, `npx tsc --noEmit` clean; boot verified — all 5 `/api/v1/appointments/*` paths registered in OpenAPI.

- **Internal in-app email module `src/email/` — email-style private messaging between main roles at `/api/v1/email`.** Fully in-DB (no external service); a message body (`email_messages`) with per-recipient copies (`email_recipients`). Recipients are restricted to the seeded "main roles" (every role except `member`: super_admin, senior_pastor, church_admin, branch_pastor, department_head, secretary, treasurer, cell_leader) within the same church, scoped by `req.profile.church_id`. Trash is a soft-delete: `EmailRecipient.deleted_at` for inbox copies, `EmailMessage.deleted_at` for the sender's sent copy; hard-deleting from trash permanently removes the row.
  - **Schema/migrations `20260830154016_add_email_messaging` + `20260830155200_add_email_recipient_relation`**: `EmailMessage` (`church_id`, `sender_id`, `sender_name?`, `subject`, `body`, `reply_to_id?`, `deleted_at?`, `@@index([church_id, created_at])`) and `EmailRecipient` (`message_id` FK → `email_messages` `onDelete: Cascade`, `profile_id`, `read_at?`, `deleted_at?`, `@@index([profile_id, read_at])`, `@@index([message_id])`).
  - **`EmailService`** (`email.service.ts`): `send` (dedupes recipient IDs, rejects self-send, validates recipients are same-church main roles, creates message + `createMany` copies, audit `CREATE` with `entity: 'email'`), `list` (inbox or sent box, paginated, `includeTrashed` flag, church-scoped via the `message` relation; returns `unreadCount` for the inbox), `getOne` (inbox copy auto-marks read; 404 when neither sender nor same-church recipient), `getUnreadCount`, `markRead`/`markUnread`, `trash`/`restore` (recipient copy vs sent message), `deleteForever` (refuses unless trashed), `listContacts` (main roles in the same church, excludes self unless `includeSelf`, filters by branch/role/search over name+email, resolves branch names, caps at 100). `MAIN_ROLES` constant exported.
  - **`EmailController`** (`Controller('email')`, `JwtAuthGuard`-only, no role ceilings — permission-gated): `GET /email/contacts` (`emails:read`, declared before the param route), `GET /email/unread-count` (`emails:read`), `GET /email` (list, `emails:read`), `POST /email` (`emails:create`, resolved `userId` from `req.user.sub` for the audit log), `GET /email/:messageId` (`emails:read`), `POST /email/:messageId/read|unread` (`emails:update`), `DELETE /email/:messageId` (trash, `emails:delete`), `POST /email/:messageId/restore` (`emails:update`), `DELETE /email/:messageId/permanent` (`emails:delete`). Registered via `EmailModule` (imports `AuthModule`) in `app.module.ts`.
  - **DTOs** `src/email/dto/`: `SendEmailDto` (recipientIds ≤50, subject ≤200, body ≤20000, replyToId? UUID), `ListEmailsDto` (`page`/`limit`/`box` enum inbox|sent/`includeTrashed`), `ListEmailContactsDto` (page/limit ≤200/search/branchId/role/includeSelf), `email-response.dto.ts` (`EmailItemDto`, `EmailDetailDto`, `EmailContactDto`, `EmailListEnvelopeDto`).
  - **Seed** (`prisma/seeds/permissions.seed.ts`): `'emails'` added to `RESOURCES` (24 resources). `emails:create/read/update/delete` → senior_pastor (via new block) + church_admin (all resources); `emails:create/read/update` → branch_pastor, secretary; `emails:read` → department_head, treasurer, cell_leader; `member` gets none. Existing databases need `npm run prisma:seed`.
  - **Tests**: new `test/unit/email/email.service.spec.ts` (Proxy Prisma mock, 20 tests) + `test/unit/email/email.controller.spec.ts` (source-scan, 13 tests: every route's `@RequirePermissions` + route-order guard). Full suite: **51 suites / 973 tests green** (was 49/942); `npm run build`, `npm run lint`, `npx tsc --noEmit` clean; boot verified (EmailModule initializes, all 10 `/api/v1/email/*` routes mapped, `Nest application successfully started`).

- **New audit-log endpoint `GET /audit` — the current user's own recent actions.** Previously the audit log was written but never exposed via any controller. This endpoint lets the web dashboard show a user their own recent activity (create/update/delete/login, etc.), scoped by their **Supabase id (`req.user.sub`)**, not the Prisma profile `id`.
  - **`src/common/audit-logs.controller.ts`** (`@Controller('audit')`): `GET /audit` → `listMyLogs` reads `church_id` from `req.profile.church_id` and `user_id` from `req.user.sub`, clamps the `?limit=` to 1–50 (default 10), and calls `AuditLoggingService.query(churchId, { userId, limit })`. `@UseGuards(JwtAuthGuard)`, `@ApiTags('Audit')`, `@ApiBearerAuth('supabase-auth')`, `@ApiListEndpoint`.
  - **DTO** `src/common/dto/audit-log-response.dto.ts`: `AuditLogItemDto` (whitelisted `id, action, entity, entityId?, ipAddress?, createdAt, entityLabel?`) + `AuditLogListResponseDto` (`data`, `total`). `entityLabel` is resolved from the row's `new_values` snapshot via `resolveEntityLabel` so the dashboard can render a friendly "Created Member — John Doe" line; raw `old_values`/`new_values`/`user_agent` are intentionally not emitted.
  - **Wiring**: controller registered in `CommonModule`. Since the controller uses `JwtAuthGuard`, `CommonModule` now also **imports `AuthModule`** (which provides `JwksService` + guard deps) — the same fix previously applied to `CustomFieldsModule`/`VisitorsModule` for "Nest can't resolve dependencies of the JwtAuthGuard". `AuditLoggingService` comes from `CommonModule` itself (it's exported there). Changelog tests/branch: `npm run build`, `npm run lint`, `npx tsc --noEmit` clean and boot verified (`Nest application successfully started`).
  - **Tests**: new `test/unit/common/audit-logs.controller.spec.ts` — source-scan spec (repo convention to avoid pulling the guard's transitive ESM-only `jose` deps into Jest) asserting the route/guard, `sub`+church scoping, limit clamping, and the whitelisted mapping. Full suite: **49 suites / 942 tests green** (was 48/938); `npm run build`, `npm run lint`, `npx tsc --noEmit` clean.

- **Branch-scoped access control via an `is_admin_hq` flag on `Profile`.** A user with `is_admin_hq = true` can read data across **all** branches (within their permission scope); without it they are restricted to data in their **own** branch (`branch_id` in their profile), and a `cell_leader` without the flag sees **only** the cell group(s) they lead. The flag is additive — it is NOT part of `role[]`, so it does not disturb `ROLE_RANK`/ceilings/permission resolution. Default is **on** at creation/promotion for `church_admin` (and super_admin); it is **never auto-cleared** when a role moves away from church_admin (left manual).
  - **Schema/migration `20260830102233_profile_is_admin_hq`**: `profiles.is_admin_hq Boolean @default(false)`.
  - **`BranchScopeService`** (`src/common/services/branch-scope.service.ts`): `ViewerScope` interface + `resolve(profile)` (→ `{ isHq, isChurchOnly, churchId, branchId, roles, memberId }`), `isBranchRestricted()`, `isVisible(scope, branchId)` (404-helper), `resolveCellGroupScope()` (without admin-hq a `cell_leader` resolves `[{ leader_id: member_id }]`). Registered + exported via `CommonModule` (`@Global()`) and `common/services/index.ts`. `request-context.middleware.ts` attaches `is_admin_hq` + `member_id` to `req.profile`; `AuthenticatedRequest.profile` type updated (`current-user.decorator.ts`).
  - **Branch-scoped reads** (all list/detail/get handlers now resolve a `viewer` from `req.profile` and filter where presence: members, admin cell groups (via `resolveCellGroupScope`), giving, attendance (rows have no `branch_id` → `where.OR = [{ service: { branch_id } }, { event: { branch_id } }]`), events, branches, profiles. A `branchId` query param is only honored for church-only (`isChurchOnly`) viewers — non-HQ viewers are forced to `scope.branchId`.
  - **Analytics scoping** (`analytics.service.ts`+controller): injected `BranchScopeService`; `getDashboard`/`getGivingAnalytics`/`getAttendanceAnalytics`/`getMemberAnalytics`/`getEventAnalytics`/`getCommunicationAnalytics` take a `viewer`; `getEngagementDistribution` scopes via `member: { branch_id }`. `/analytics/*` role ceilings unchanged (church_admin/senior_pastor/branch_pastor; giving +treasurer); admin-hq defaults on only for church_admin, so non-HQ senior/branch pastors see their own branch only. Super_admin dashboard must not call `/analytics/*` (would 403).
  - **`is_admin_hq` defaults**: register (`auth.service.ts`: `is_admin_hq: true` when `dto.role === 'church_admin'`), profile invite + church staff invite (`is_admin_hq: dto.role === 'church_admin'`), role update (`orderedRoles.includes('church_admin') ? { is_admin_hq: true } : {}`), `updateProfileRoles` (same rule).
  - **DTO + mapping**: `AdminUpdateUserDto.isAdminHq?: boolean` (`@IsBoolean`); `adminUpdateProfile` persists when `!== undefined`; `ProfileResponseDto.isAdminHq!: boolean` + `mapToResponseDto` emits it. All top-level `profile` queries use `include`, so all scalars return.
  - **Tests**: new `test/unit/common/branch-scope.service.spec.ts` (13 tests) plus scoping/default tests in members/admin/giving/analytics/auth/profile specs. Full suite: **48 suites / 938 tests green** (was 917); `npm run build` + `npm run lint` clean; `npx tsc --noEmit` clean.
...
- **Email-OTP two-factor authentication (2FA) replacing TOTP MFA, with per-profile self-service + login gating.** Two-factor is now a **per-user** setting managed ONLY on the `/profile` page (the web admin Security Settings tab will drop its static MFA badge — see web changelog). Login for an enabled account is staged: the API issues **no token** until an emailed 6-digit code is verified via a dedicated endpoint.
  ...
  - **ProfileService** (`sendTwoFactorCode`, `toggleTwoFactor`, `getTwoFactorEnabled`, `getTwoFactorKey`): emails a 6-digit code via `ResendService` (30s cooldown, 10-min TTL), stores the SHA-256 digest + attempt counter in Redis (`2fa:{purpose}:{userId}`), verifies with max 5 attempts, then flips `two_factor_enabled` and audit-logs. Removed the entire legacy TOTP flow (`generateMfaSecret`/`enableMfa`/`disableMfa`, `otplib` dependency, `getMfaSecretKey`) which had a latent bug (enable deleted the Redis secret that disable re-read, so disable always failed). `ProfileResponseDto`/`mapToResponseDto` now expose both `mfaEnabled` and `twoFactorEnabled`.
  - **ProfileController**: replaced `me/mfa/generate|enable|disable` with `POST me/2fa/enable-code`, `POST me/2fa/enable`, `POST me/2fa/disable-code`, `POST me/2fa/disable`, `POST me/2fa/resend` (all auth-only, verify via new `VerifyOtpDto { code }`). Deleted `verify-mfa.dto.ts`/`mfa-secret-response.dto.ts`; `src/profile/index.ts` now exports `verify-otp.dto`. `ProfileModule` imports `CommunicationModule` for `ResendService`.
  - **Login gating** (`auth.service.ts`/`auth.controller.ts`): `login()` fetches the profile with `two_factor_enabled` + church name; when enabled it stores the (**uncommitted**) Supabase session in Redis (`2fa:login:pending:{userId}`, 10-min TTL), emails an OTP, and returns a **staged** `LoginResponseDto` (`{ requiresTwoFactor: true, twoFactorEmail, userId }`) with **no accessToken**. New public `POST /auth/login/2fa` (`Login2faDto { email, code }`) verifies the code against the pending digest (max 5 attempts) and, on success, deletes the pending record and returns the full session (accessToken/refreshToken/profile). `LoginResponseDto` fields `accessToken`/`expiresAt`/`email` are now optional + new optional `requiresTwoFactor`/`twoFactorEmail`. `AuthModule` imports `CommunicationModule` (non-circular — CommunicationModule imports nothing).
  - **Tests**: profile.service.spec — MFA block replaced with 2FA lifecycle (send code + redis digest + resend email, enable via verify, wrong-code rejection, `getTwoFactorEnabled` found/not-found); auth.service.spec — login stages OTP when 2FA enabled, `completeTwoFactorLogin` success/wrong-code/no-pending. Full suite: **47 suites / 917 tests green**; build + lint clean; boot verified (`Nest application successfully started`).

- **Templates + branches list endpoints accept `limit` up to 200 (fixes 400 on the web broadcast-new page dropdowns).** The web Communication "New Broadcast" page fetches templates and branches with `limit: 200` for its dropdowns, but `ListTemplatesDto.limit` and `ListBranchesDto.limit` both capped at `@Max(100)` — so `GET /templates?status=published&limit=200` and `GET /branches?limit=200` returned 400 from the global ValidationPipe, and the dropdowns rendered empty. Worse, `ListTemplatesDto`'s `page`/`limit` lacked `@Type(() => Number)`, so the string query value (`"200"`) failed `@IsInt()` before the max was even evaluated. Fixed: added `@Type(() => Number)` to `ListTemplatesDto.page`/`limit` and raised both modules' `limit` cap to `@Max(200)`, matching the established fetch-all convention already used by giving/visitors/attendance. No frontend change needed. Build + lint clean; templates + branches unit suites green (57 tests).

- **Notifications: server-side list filters (read/date) + bulk mark-read/delete** (web drawer filters + multi-select shipped this round — see web changelog).
  - **`listNotifications` extended**: signature is now `(churchId, profileId, page=1, limit=20, type?, read?, startDate?, endDate?)`. `read === 'unread'` adds `read_at: null` to the `where`; `startDate`/`endDate` (ISO `yyyy-MM-dd`) add a `created_at` range filter (`gte` at `T00:00:00.000Z`, `lte` at `T23:59:59.999Z`) so day bounds are inclusive. Controller accepts `@Query('read'|'startDate'|'endDate')` and forwards them.
  - **New bulk endpoints** (declared before the `:notificationId` param route): `PATCH /notifications/bulk-read` and `POST /notifications/bulk-delete` (`@HttpCode(200)`), both taking a validated `BulkNotificationsDto` (`ids: string[]`, UUIDs, `@ArrayNotEmpty`/`@ArrayMaxSize(200)`). `NotificationsService.bulkMarkAsRead(churchId, profileId, ids)` → `updateMany` (id `in ids`, church+profile scoped, `read_at: null` only) returns `{ updated }`; `bulkRemove(churchId, profileId, ids)` → `deleteMany` (church+profile scoped) returns `{ deleted }`.
  - **Tests**: `notifications.service.spec.ts` +5 (unread filter, date-range bounds, `bulkMarkAsRead` scoping + `read_at: null` guard, `bulkRemove` scoping) and the Prisma mock gained `deleteMany`. Full suite: **47 suites / 910 tests green** (was 906); `npm run build` + `npm run lint` clean; boot verified (both bulk routes mapped, `Nest application successfully started`).

- **In-app notifications: auto-generated CRUD activity feed for the acting user + profile-id bug fix + single/detail endpoints.** Backend counterpart of the web header bell (see web changelog). Every user mutation now mirrors into an in-app notification for the acting user, shown in the web header bell dropdown / "View all" drawer / detail dialog.
  - **`notifications` module expanded**: `NotificationsController` rewritten to resolve the acting profile via `getProfile(req).id` **consistently** (was a mix: service layer correctly used `profileId`, but the controller read `user.sub`/`user.id` wrong — so unread-count was always 0 and mark-as-read always 404'd since `Profile.user_id` (Supabase `sub`) ≠ Prisma `profile.id`). Added `GET /notifications/:notificationId` (`getOne`, church+profile scoped, 404 when not the user's) and `DELETE /notifications/:notificationId` (**hard delete**, `@HttpCode(200)`, returns `{ success: true }`); removed now-unused `CurrentUser`/`SupabaseJwtPayload` imports. `NotificationsService` gains matching `getOne` + `remove` methods (church+profile scoped, `NotFoundException` when missing/not-own, `remove` = `prisma.notification.delete`).
  - **`AuditLoggingService.log()` now auto-notifies via the global audit path** — no per-module call sites. After writing the audit row it creates a `system` notification for the acting user for `CREATE`/`UPDATE`/`DELETE`/`ARCHIVE`/`RESTORE` only (**not** `LOGIN`/`LOGOUT`/`EXPORT`, which are reads/non-CRUD and thus excluded). It resolves the profile by `user_id`, builds a friendly title ("Created Member", "Updated Giving Category", …) and body using a name pulled from `newValues` (falls back to a humanized entity label), attaches `{ entity, entityId, action }` as `data`, and swallows any failure in a try/catch so a notification problem can never break the underlying CRUD. Existing business notifications (giving receipts, broadcast, member conversions, risk alerts) are untouched.
  - **Circular-dependency fix (runtime crash)**: `NotificationsService` injects `WhatsAppService`, and `WhatsAppService` injects `AuditLoggingService`, so a direct `NotificationsService` constructor dependency in `AuditLoggingService` created a **file-level import cycle** (`audit-logging → notifications → whatsapp → audit-logging`) that corrupted Nest's reflected constructor metadata — `WhatsAppService`'s `design:paramtypes` captured `AuditLoggingService` as `undefined` (index [2]) → "Nest can't resolve dependencies of the WhatsAppService … circular import" at boot (not caught by `tsc`/unit tests, which construct services directly). Resolved by breaking the reference without a static import of `NotificationsService`: new `src/notifications/notification-tokens.ts` exports `NOTIFICATIONS_SERVICE_TOKEN` (a Symbol that imports nothing); `NotificationsModule` registers `{ provide: NOTIFICATIONS_SERVICE_TOKEN, useExisting: NotificationsService }` (and exports the token alongside the class); `AuditLoggingService` injects `ModuleRef` (not `NotificationsService`) and lazily resolves `this.moduleRef.get(NOTIFICATIONS_SERVICE_TOKEN, { strict: false })` on first CRUD-notify call (cached). Boot verified end-to-end: `WhatsAppModule`/`NotificationsModule` initialize, all 7 notification routes map, no DI error (the only boot exit under `timeout` was `EADDRINUSE` — the dev server already occupies port 3001).
  - **Tests**: new `test/unit/common/audit-logging.service.spec.ts` (6 tests — writes the audit row; creates a CREATE notification resolving `user_id → profile.id` via a `ModuleRef` mock; creates UPDATE/DELETE/ARCHIVE/RESTORE notifications; skips LOGIN/LOGOUT/EXPORT; skips when no profile; swallows notification failures). `notifications.service.spec.ts` +4 (`getOne` found/not-own-404, `remove` hard-delete/not-own-404 with no delete call). Full suite: **47 suites / 906 tests green** (was 46/896); `npm run build` + `npm run lint` clean.
...
- **Forms double-submission guard + shareable-link regeneration** (the web dynamic Form Builder /forms + public submit pages shipped this round too — see web changelog):
  ...
  - **`submitForm` (authenticated)**: rejects a second submission by the same user with `ConflictException('You have already submitted this form')`.
  - **`submitByPublicToken` (anonymous)**: enforces the submission-level cap when `submission_limit > 0` (`ConflictException` with the max count) and, when `unique_field` is set, scans existing submissions for a matching value and 409s duplicates with a friendly field-label message. Both guards run before the audit-logged create.
  - **`regeneratePublicToken(churchId, formId, userId)`**: new service method that issues a fresh `public_token` (audit `UPDATE`); `Conflict` when the form isn't public, `NotFound` when archived. New endpoint `POST /forms/:formId/regenerate-link` (`church_admin`/`branch_pastor`/`secretary`, `@HttpCode(OK)`).
  - **Public form metadata**: new `getPublicFormMeta(publicToken)` service method + `PublicFormMetaDto` + `GET /forms/public/:publicToken/meta` on the auth-free `FormsPublicController` — returns only the published public form's `title`, `description?`, and `fields` (no church or submission data) so the anonymous web renderer can build the form; `NotFound` when the token is invalid or the form isn't public/published/archived.
  - **`createForm`/`updateForm`/`mapForm`/`formToPlain`** now persist and surface `unique_field`/`submission_limit`.
  - **Tests**: forms.service.spec +7 (auth duplicate rejection, public submission-limit rejection, public unique-field duplicate rejection, regenerate success/not-public-conflict/archived-not-found). Full suite: **46 suites / 884 tests green**; `npm run build` + `npm run lint` clean.
- **Forms round 2 — public attachments, submit counts, and close/reopen lifecycle** (the web member-fill page, attachments component, counts, and Close/Reopen buttons shipped this round too — see web changelog):
  - **Public submissions now persist file attachments**: `submitByPublicToken` previously hard-coded `attachments: []` and ignored `attachmentAssetIds`; it now resolves them through the same church-scoped `resolveAttachments` (which needs no actor) so anonymous respondents' uploaded files land on the submission.
  - **`closeForm` / `reopenForm`**: new service methods that flip a form's status to `closed` (Conflict if already closed/archived) and back to `draft` (Conflict unless currently `closed`), both audit-logged `UPDATE`. New endpoints `POST /forms/:formId/close` and `POST /forms/:formId/reopen` (`WRITE_ROLES` + `@HttpCode(OK)`).
  - **`FormResponseDto.submissionCount`** (number, default 0): `mapForm` now reads `form._count.submissions` (`?? 0`), `findFormOrFail` includes `_count: { select: { submissions: true } }` so every detail/mutation response carries the live count, and `listForms.findMany` includes it so list rows do too — the web list/detail pages can now show accurate counts instead of client-side estimates.
  - **Media upload response now exposes `assetId`** (supporting change): `MediaResponseDto` gains `assetId`, and both `uploadImage`/`uploadFile` capture the created `mediaAsset.id` and return it (audit `entityId` switches from the filename to the asset ID) — the web submission-attachments control needs the asset ID to pass `attachmentAssetIds`.
  - **Tests**: forms.service.spec +11 total this round — `getPublicFormMeta` ×5 (valid returns title/description/fields, invalid-token / non-public / non-published / archived → `NotFound`), public-submission `attachmentAssetIds` persistence (resolves via `mediaAsset.findMany`, maps onto the created submission), `closeForm` ×3 (success, already-closed Conflict, archived Conflict), `reopenForm` ×2 (success → draft, not-closed Conflict). Full suite: **46 suites / 895 tests green**; `npm run build` + `npm run lint` clean.
