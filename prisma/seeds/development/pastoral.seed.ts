...
export async function seedPastoral(
  ...
  for (const note of notes) {
    const [memberIndex, content, confidentiality] = note;
    ...
    const existing = await prisma.pastoralNote.findFirst({
      where: {
        church_id: churchId,
        member_id: member.id,
        content,
  ...
  for (const risk of risks) {
    const [memberIndex, score, level] = risk;
    const member = members[memberIndex];
  ...
  for (const engagement of engagements) {
    const [memberIndex, score] = engagement;
    const member = members[memberIndex];
    ...
    const existing = await prisma.engagementScore.findFirst({
      where: {
        member_id: member.id,
      },
    });
