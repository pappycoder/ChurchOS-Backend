...
export class CreateLifeEventDto {
  // Require a valid member ID to associate the life event with
  @ApiProperty({
  ...
  // Require a recognized life event type
  @ApiProperty({
  ...
  // Require a valid ISO date string for the event
  @ApiProperty({
  ...
  // Allow optional structured details as a JSON object
  @ApiPropertyOptional({
