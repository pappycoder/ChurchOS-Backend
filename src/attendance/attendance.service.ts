...
export class AttendanceService {
  ...
  async recordAttendance(
    ...
    if (!dto.memberId && !dto.visitorName) {
      throw new BadRequestException('Either memberId or visitorName must be provided');
    }
    ...
    const attendance = await this.prisma.attendance.create({
      data: {
        ...
        visitor_name: dto.visitorName || null,
  ...
  async recordBulkAttendance(
    ...
    for (let i = 0; i < dto.records.length; i++) {
      ...
      if (!record.memberId && !record.visitorName) {
        errors.push({ index: i, message: 'Either memberId or visitorName must be provided' });
        continue;
      }
  ...
  async recordVisitorAttendance(
    dto: { serviceId: string; visitorName: string; source?: string },
    churchId: string,
    userId: string,
  ): Promise<AttendanceResponseDto> {
  ...
  async getAttendanceSummary(
    ...
    const [totalCheckIns, memberCheckIns, visitorCheckIns, bySourceRaw] = await Promise.all([
    ...
    return { totalCheckIns, memberCheckIns, visitorCheckIns, bySource };
  }
